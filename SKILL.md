...
8. **Architecture diagram.** Use Mermaid `graph LR` (left-to-right) by default — a slide is 16:9, so a horizontal flow fills the width; top-down (`graph TD`) leaves the middle and right empty. Only switch to `TD` when the flow is genuinely deep (many layers) rather than a linear chain. Widen `flowchart.rankSpacing` (~70) for the horizontal layers, reorder nodes so edges sharing a target don't cross, and let the diagram fill the slide: `.mermaid-wrap .mermaid svg { max-width: 100% !important; width: 100%; height: auto; max-height: 62vh; }` (the `!important` overrides Mermaid's inline max-width cap so the SVG scales up instead of stopping at its natural width; aspect ratio is preserved and `max-height` keeps tall diagrams in frame).
